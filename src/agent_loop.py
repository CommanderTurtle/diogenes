...
For LONG-running commands (uv/Bun package installs, ffmpeg, model downloads, training, builds — anything that may take more than ~20s), make the FIRST line `#!bg` to run it in the BACKGROUND. You get a job id back immediately and are automatically re-invoked with the full output when it finishes — so you never block the chat waiting. Use `uv pip --python` for Python packages and Bun for JavaScript packages; never assume bare pip, Node, npm, or pnpm exists. Example:
```bash
#!bg
uv pip install --python "$VIRTUAL_ENV/bin/python" openai-whisper
```
