...
import {
  colibriSettingsFromFields,
  renderColibriCommand,
  synchronizeColibriLaunchPort,
} from './cookbookColibriLaunch.js';
import {
  prismModelIdForIdentity,
  prismSettingsFromFields,
  renderPrismCommand,
  synchronizePrismLaunchPort,
} from './cookbookPrismLaunch.js';
...
let _prismProvidersCache = null;
let _prismProvidersCachedAt = 0;
...
async function _prismProviderForModel(model, force = false) {
  const modelId = prismModelIdForIdentity(model);
  if (!modelId) return null;
  if (force || !_prismProvidersCache || Date.now() - _prismProvidersCachedAt > 15000) {
    const res = await fetch('/api/odysseus/prism/providers', { credentials: 'same-origin' });
    if (!res.ok) throw new Error(`PrismML provider observation failed: HTTP ${res.status}`);
    const data = await res.json();
    _prismProvidersCache = Array.isArray(data.providers) ? data.providers : [];
    _prismProvidersCachedAt = Date.now();
  }
  const provider = _prismProvidersCache.find(item => item.id === 'prism.llamacpp') || null;
  if (!provider) return null;
  const modelReport = (provider.models || []).find(item => item.id === modelId) || null;
  return modelReport ? { provider, model: modelReport } : null;
}
...
function _formatServeCmdPreview(cmd) {
  ...
  if (mlxDeepSeekV4Compat) {
    return [
      '# Ɗiogenēs runtime compatibility: using sanitized MLX DeepSeek-V4 shim.',
      formatted,
    ].join('\n');
  }
  return formatted;
...
function _backendChoicesForTarget(
  target,
  model = null,
  colibriProvider = null,
  prismRuntime = null,
) {
  ...
  if (target?.host || target?.platform === 'windows') return standard;
  const native = [];
  if (prismRuntime && prismModelIdForIdentity(model)) {
    native.push(['prism', 'PrismML']);
  }
  const runtimeId = _colibriRuntimeIdForModel(model);
  const choice = runtimeId === 'colibri.glm'
    ...
    : null;
  if (choice && colibriProvider) native.push(choice);
  return native.length ? [...native, ...standard] : standard;
}
...
async function _fetchServeRuntimePackage(panel, backend) {
  ...
  if (backend === 'prism') {
    ...
    const runtime = await _prismProviderForModel(
      { repo_id: card?.dataset?.repo || '' },
      true,
    );
    const target = _selectedServeTarget(panel);
    if (!runtime) return { pkg: null, target };
    const { provider, model } = runtime;
    const exactWeightsReady = !!model.weights?.complete;
    const selectedPort = Number(panel.querySelector('[data-field="port"]')?.value || 0);
    const observedPort = Number(provider.endpoint?.port || 0);
    const portAvailable = !(
      provider.endpoint?.port_open
      && selectedPort === observedPort
    );
    const effectivePort = selectedPort || observedPort || 'configured';
    const summary = [
      provider.source?.ready ? 'official prism source ready' : 'source not ready',
      provider.build?.ready ? 'CUDA 13 / sm_120a build ready' : 'CUDA 13 / sm_120a build required',
      exactWeightsReady ? `${model.weights.path} ready` : `${model.weights?.path || 'exact GGUF'} missing or incomplete`,
      portAvailable ? `port ${effectivePort} available` : `port ${effectivePort} occupied`,
    ].join('; ');
    return {
      pkg: {
        name: 'prism',
        installed: !!(
          provider.source?.ready
          && provider.build?.ready
          && exactWeightsReady
          && portAvailable
        ),
        probe_error: false,
...
function _runtimeNoteText(backend, pkg, target) {
  const labels = { vllm: 'vLLM', sglang: 'SGLang', llamacpp: 'llama.cpp', mlx: 'MLX', mlx_image: 'MLX Image', diffusers: 'Diffusers', colibri_glm: 'Colibri · GLM', colibri_hy3: 'Colibri · Hy3', prism: 'PrismML' };
...
function _rerenderCachedModels() {
  ...
  list.querySelectorAll('.memory-item[data-repo]').forEach(item => {
    item.addEventListener('click', async (e) => {
      ...
      let _prismRuntime = null;
      if (prismModelIdForIdentity(m)) {
        try {
          _prismRuntime = await _prismProviderForModel(m);
        } catch (error) {
          console.warn('[cookbook] PrismML provider observation unavailable', error);
        }
      }
      ...
      const _prismProvider = _prismRuntime?.provider || null;
      const _prismModel = _prismRuntime?.model || null;
      const _prismProfiles = _prismProvider?.profiles || {};
      const _prismProfileId = String(
        sv('prism_profile', _prismProvider?.default_profile || 'rtx5090-quality')
      );
      const _prismProfile = _prismProfiles[_prismProfileId] || {};
      const _prismProfileOptions = Object.entries(_prismProfiles).map(([id, profile]) =>
        `<option value="${esc(id)}"${id === _prismProfileId ? ' selected' : ''}>${esc(profile.label || id)}</option>`
      ).join('');
      ...
      if (_colibriProvider) {
        ...
        panelHtml += `<input type="hidden" class="hwfit-sf" data-field="colibri_profile_env" value="${esc(JSON.stringify(_colibriProfile.env || {}))}" />`;
      }
      if (_prismProvider && _prismModel) {
        panelHtml += `<input type="hidden" class="hwfit-sf" data-field="prism_provider_id" value="${esc(_prismProvider.id)}" />`;
        panelHtml += `<input type="hidden" class="hwfit-sf" data-field="prism_model_id" value="${esc(_prismModel.id)}" />`;
        panelHtml += `<input type="hidden" class="hwfit-sf" data-field="prism_api_model_id" value="${esc(_prismModel.api_model_id || '')}" />`;
        panelHtml += `<input type="hidden" class="hwfit-sf" data-field="prism_host" value="0.0.0.0" />`;
        panelHtml += `<input type="hidden" class="hwfit-sf" data-field="prism_command" value="" />`;
      }
      ...
      if (_prismProvider) {
        panelHtml += `<label class="hwfit-backend-prism">${_l('Profile','Server-validated PrismML launch profile. Experimental DSpark, vision, and KV4 remain explicit opt-ins.')}<select class="hwfit-sf" data-field="prism_profile">${_prismProfileOptions}</select></label>`;
      }
      const defaultPort = _prismProvider?.endpoint?.port
        ? String(_prismProvider.endpoint.port)
      ...
      if (_colibriProvider) {
        ...
        panelHtml += `<label>${_l('Expert VRAM GB','GPU memory requested for the resident expert tier. It is not a total-card cap: the grouped GLM checkpoint measures about 12.48 GB of dense tensors before KV state, CUDA workspaces, and reserve. Diogenes therefore requests 17 GB on a 32 GB 5090; Hy3 keeps its dense stack on CPU and can devote up to 28 GB to experts.')}<input type="text" class="hwfit-sf" data-field="colibri_vram" value="${esc(sv('colibri_vram', _profileVram))}" /></label>`;
      ...
      if (_prismProvider && _prismModel) {
        panelHtml += `<div class="hwfit-serve-row hwfit-serve-row-core hwfit-backend-prism">`;
        panelHtml += `<label>${_l('Context','PrismML context allocation. RTX 5090 quality defaults to 131072; use the Portable profile for a conservative 8192-token start.')}<input type="text" class="hwfit-sf" data-field="prism_context" value="${esc(sv('prism_context', String(_prismProfile.context ?? 131072)))}" /></label>`;
        panelHtml += `<label>${_l('GPU Layers','llama.cpp GPU offload layer budget. 999 requests full offload and is the managed CUDA default.')}<input type="text" class="hwfit-sf" data-field="prism_gpu_layers" value="${esc(sv('prism_gpu_layers', String(_prismProfile.gpu_layers ?? 999)))}" /></label>`;
        panelHtml += `<label>${_l('Parallel','Independent llama-server slots. DSpark speculative decoding requires exactly one.')}<input type="text" class="hwfit-sf" data-field="prism_parallel" value="${esc(sv('prism_parallel', String(_prismProfile.parallel ?? 1)))}" /></label>`;
        panelHtml += `</div>`;
      }
      ...
      if (_colibriProvider) {
        ...
        const _cudaPipeDefault = String(_colibriEnv.COLI_CUDA_PIPE ?? '0');
        const _ioOptions = _colibriIsGlm ? ['0','1'] : ['0','1','2'];
        ...
        panelHtml += `<label>${_l('I/O Pipeline',_colibriIsGlm ? 'GLM expert reads support synchronous mode 0 or the thread-pool pipeline in mode 1. GLM io_uring is a separate switch.' : 'Hy3 expert-read pipeline: 0 is synchronous, 1 is the thread pool, and 2 uses io_uring when the IOURING build is available, falling back to mode 1 otherwise.')}<select class="hwfit-sf" data-field="colibri_io_pipeline">${_ioOptions.map(value => `<option value="${value}"${String(sv('colibri_io_pipeline', _ioDefault)) === value ? ' selected' : ''}>${value}</option>`).join('')}</select></label>`;
        panelHtml += `<label>${_l('I/O Workers','Concurrent expert-weight readers. Upstream defaults to 8; the RTX 5090 + local NVMe profiles use 16 explicitly, within Colibri’s supported 1–64 range.')}<input type="text" class="hwfit-sf" data-field="colibri_pipe_workers" value="${esc(sv('colibri_pipe_workers', String(_colibriEnv.PIPE_WORKERS || '8')))}" /></label>`;
        if (_colibriIsGlm) panelHtml += `<label class="hwfit-backend-colibri_glm">${_l('CUDA Pipeline','CUDA execution mode: 0 disables the pipeline, 1 uses resident multi-step execution, and 2 uses the single-GPU pipe2 path measured in GLM issue #273. This is unrelated to io_uring.')}<select class="hwfit-sf" data-field="colibri_cuda_pipeline">${['0','1','2'].map(value => `<option value="${value}"${String(sv('colibri_cuda_pipeline', _cudaPipeDefault)) === value ? ' selected' : ''}>${value}</option>`).join('')}</select></label>`;
        panelHtml += `<label>${_l('Repin','Recalculate the resident RAM/VRAM expert set after this many tokens. The 5090/Gen5 profiles use the measured 16-token cadence; blank preserves upstream default.')}<input type="text" class="hwfit-sf" data-field="colibri_repin" value="${esc(sv('colibri_repin', _colibriProfile.repin ?? ''))}" placeholder="auto" /></label>`;
        ...
        panelHtml += `<label>${_l('KV Slots','Independent sequence contexts. GLM MTP requires exactly one slot; Diogenes rejects an MTP command with any other value.')}<input type="text" class="hwfit-sf" data-field="colibri_kv_slots" value="${esc(sv('colibri_kv_slots', '1'))}" /></label>`;
        panelHtml += `</div>`;
        panelHtml += `<div class="hwfit-serve-row hwfit-backend-colibri_glm hwfit-backend-colibri_hy3">`;
        panelHtml += `<label>${_l('Expert Top-P','Colibri CLI --topp prunes the cumulative MoE expert distribution. It is not OpenAI token top_p and changes model routing/quality. Faithful profiles leave it blank; set token top_p per API request instead.')}<input type="text" class="hwfit-sf" data-field="colibri_topp" value="${esc(sv('colibri_topp', ''))}" placeholder="faithful" /></label>`;
        panelHtml += `<label>${_l('Expert Top-K','Colibri CLI --topk limits how many MoE experts run. It is not token-sampling top_k and changes model quality. Faithful profiles leave it blank.')}<input type="text" class="hwfit-sf" data-field="colibri_topk" value="${esc(sv('colibri_topk', ''))}" placeholder="faithful" /></label>`;
        panelHtml += `<label>${_l('Fallback Temperature','Server-side sampling fallback. OpenAI-compatible requests may override it; set request temperature/top_p in the client for normal chat tuning. Blank preserves upstream behavior.')}<input type="text" class="hwfit-sf" data-field="colibri_temp" value="${esc(sv('colibri_temp', ''))}" placeholder="request-owned" /></label>`;
        ...
        if (_colibriIsGlm) panelHtml += `<label class="hwfit-sf-cb hwfit-backend-colibri_glm"><input type="checkbox" class="hwfit-sf" data-field="colibri_pilot_real"${sv('colibri_pilot_real', _colibriEnv.PILOT_REAL === '1') ? ' checked' : ''} /> Real Prefetch${_h('Enable the GLM value-preserving cross-layer expert prefetch path. Hy3 does not implement this knob.')}</label>`;
        if (_colibriIsGlm) panelHtml += `<label class="hwfit-sf-cb hwfit-backend-colibri_glm"><input type="checkbox" class="hwfit-sf" data-field="colibri_uring"${sv('colibri_uring', _colibriEnv.URING === '1') ? ' checked' : ''} /> io_uring A/B${_h('Enable GLM’s separate Linux io_uring expert reader. This implies pipelined direct I/O and is independent of CUDA pipe2; benchmark it on the local native filesystem before keeping it.')}</label>`;
        if (_colibriIsGlm) panelHtml += `<label class="hwfit-sf-cb hwfit-backend-colibri_glm"><input type="checkbox" class="hwfit-sf" data-field="colibri_cache_route"${sv('colibri_cache_route', _colibriEnv.CACHE_ROUTE === '1') ? ' checked' : ''} /> Approximate Cache Route${_h('May replace lower-ranked routed experts with resident experts. This changes model routing and is never part of the faithful preset; issue #273 throughput figures use it only as an explicit quality trade-off.')}</label>`;
        panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="colibri_cuda_mtp"${sv('colibri_cuda_mtp', _colibriIsGlm ? _colibriEnv.COLI_CUDA_MTP === '1' : Number(_colibriEnv.DRAFT ?? 3) > 0) ? ' checked' : ''} /> ${_colibriIsGlm ? 'MTP Trial' : 'MTP'}${_h(_colibriIsGlm ? 'Use the checkpoint’s included int8 MTP head with the current one-token draft depth and pinned speculative buffers. The exact single-5090 report is faster with MTP off, so this remains an explicit A/B.' : 'Use the checkpoint’s six native Hy3 MTP shards. Linear three-token drafting is the current default; the engine automatically disables it after persistently low acceptance.')}</label>`;
        if (_colibriIsGlm) panelHtml += `<label class="hwfit-sf-cb hwfit-backend-colibri_glm"><input type="checkbox" class="hwfit-sf" data-field="colibri_tensor_cores"${sv('colibri_tensor_cores', _colibriEnv.COLI_CUDA_TC_W4A16 === '1') ? ' checked' : ''} /> W4A16 Tensor Cores${_h('Enables the upstream W4A16 tensor-core path for prefill and batches of at least 16 rows. It is normally neutral during single-token decode and is enabled in the 5090 profiles for faster prompt ingestion.')}</label>`;
        if (!_colibriIsGlm) panelHtml += `<label class="hwfit-sf-cb hwfit-backend-colibri_hy3"><input type="checkbox" class="hwfit-sf" data-field="colibri_cuda_dense"${sv('colibri_cuda_dense', _colibriEnv.CUDA_DENSE === '1') ? ' checked' : ''} /> CUDA Dense${_h('Keep Hy3 dense layers on GPU. This leaves less card memory for the expert tier and KV state; compare against the expert-cache-first profile on this host.')}</label>`;
        if (!_colibriIsGlm) panelHtml += `<label class="hwfit-sf-cb hwfit-backend-colibri_hy3"><input type="checkbox" class="hwfit-sf" data-field="colibri_cuda_attention"${sv('colibri_cuda_attention', _colibriEnv.CUDA_ATTN === '1') ? ' checked' : ''} /> CUDA Attention${_h('Best with float KV. Enabling KV-I8 disables this path because the current CUDA attention kernel expects float KV.')}</label>`;
        if (!_colibriIsGlm) panelHtml += `<label class="hwfit-sf-cb hwfit-backend-colibri_hy3"><input type="checkbox" class="hwfit-sf" data-field="colibri_kv_i8"${sv('colibri_kv_i8', _colibriEnv.KV_I8 === '1') ? ' checked' : ''} /> KV-I8${_h('Compress KV memory for longer context, but fall back from CUDA attention. This is a memory/quality trade-off and is not enabled in the faithful preset.')}</label>`;
        panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="colibri_tool_salvage"${sv('colibri_tool_salvage', _colibriEnv.COLI_TOOL_SALVAGE === '1') ? ' checked' : ''} /> Tool Salvage${_h('Recover malformed tool-call envelopes at the Colibri gateway. Supported by both current GLM and Hy3 runtimes and enabled in faithful profiles.')}</label>`;
        if (!_colibriIsGlm) panelHtml += `<label class="hwfit-sf-cb hwfit-backend-colibri_hy3"><input type="checkbox" class="hwfit-sf" data-field="colibri_verbose"${sv('colibri_verbose', false) ? ' checked' : ''} /> Diagnostics${_h('Enable verbose Hy3 output plus PERF telemetry every 100 tokens. Off by default because it is intentionally noisy.')}</label>`;
        panelHtml += `</div>`;
        panelHtml += `<div class="hwfit-serve-row hwfit-backend-colibri_glm">`;
        ...
        panelHtml += `<label>${_l('Route Alpha','Blend used by approximate cache routing (0–1). Upstream Blackwell measurements use 0.5.')}<input type="text" class="hwfit-sf" data-field="colibri_route_alpha" value="${esc(sv('colibri_route_alpha', '0.5'))}" /></label>`;
        panelHtml += `</div>`;
      }
      if (_prismProvider && _prismModel) {
        panelHtml += `<div class="hwfit-backend-prism" style="font-size:10px;line-height:1.45;color:var(--fg-muted);margin:2px 0 8px;padding:7px 9px;border:1px solid var(--border);border-radius:6px;">${esc(_prismProfile.description || '')}<br><span style="opacity:.8;">Source: ${esc(_prismProvider.source?.commit || 'not observed')} · exact model: ${esc(_prismModel.weights?.path || 'not configured')}</span></div>`;
        panelHtml += `<div class="hwfit-serve-row hwfit-backend-prism">`;
        panelHtml += `<label>${_l('Temperature','Sampling temperature accepted by the managed PrismML command renderer (0–2).')}<input type="text" class="hwfit-sf" data-field="prism_temperature" value="${esc(sv('prism_temperature', '0.7'))}" /></label>`;
        panelHtml += `<label>${_l('Top P','Nucleus sampling cutoff (0–1).')}<input type="text" class="hwfit-sf" data-field="prism_top_p" value="${esc(sv('prism_top_p', '0.95'))}" /></label>`;
        panelHtml += `<label>${_l('Top K','Maximum top candidates (0–1000).')}<input type="text" class="hwfit-sf" data-field="prism_top_k" value="${esc(sv('prism_top_k', '20'))}" /></label>`;
        panelHtml += `<label>${_l('Min P','Minimum relative probability cutoff (0–1).')}<input type="text" class="hwfit-sf" data-field="prism_min_p" value="${esc(sv('prism_min_p', '0'))}" /></label>`;
        panelHtml += `<label>${_l('Reasoning Budget','Optional llama-server reasoning budget. Blank omits the flag; -1 permits the runtime maximum.')}<input type="text" class="hwfit-sf" data-field="prism_reasoning_budget" value="${esc(sv('prism_reasoning_budget', ''))}" placeholder="runtime default" /></label>`;
        panelHtml += `</div>`;
        panelHtml += `<div class="hwfit-serve-checks hwfit-backend-prism">`;
        panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="prism_flash_attention"${sv('prism_flash_attention', _prismProfile.flash_attention !== false) ? ' checked' : ''} /> Flash Attention${_h('Use the Prism CUDA fork Flash Attention path. Enabled in both committed profiles.')}</label>`;
        panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="prism_kv4"${sv('prism_kv4', !!_prismProfile.kv4) ? ' checked' : ''} /> KV4 Cache${_h('Quantize both K and V caches to q4_0. This reduces KV memory but is an explicit quality trade-off.')}</label>`;
        panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="prism_speculative"${sv('prism_speculative', !!_prismProfile.speculative) ? ' checked' : ''} /> DSpark${_h('Enable the exact model-matched DSpark drafter. Requires its exact GGUF, one server slot, and at least 16384 context. Disabled by default.')}</label>`;
        panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="prism_vision"${sv('prism_vision', false) ? ' checked' : ''} /> Vision${_h('Attach the exact model-matched mmproj projector. Disabled by default and launch-gated until that exact file is complete.')}</label>`;
        panelHtml += `</div>`;
      }
      ...
      panelHtml += `<label class="hwfit-backend-vllm" style="grid-column:1 / -1;">${_l('Model Path','Argument passed after `vllm serve`. Use a repository ID or an explicit local directory. For MiniMax M3, Ɗiogenēs resolves an existing local Hugging Face snapshot when one is available.')}<input type="text" class="hwfit-sf" data-field="model_path" value="${esc(_modelPathValue)}" placeholder="${esc(repo)}" style="width:100%;" /></label>`;
      ...
      panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="prefix_cache"${sv('prefix_cache',false)?' checked':''} /> Prefix / Radix Cache${_h('vLLM: prefix caching. SGLang: RadixAttention prefix cache; when off Ɗiogenēs adds --disable-radix-cache.')}</label>`;
      ...
      panelHtml += `<div class="hwfit-serve-extra hwfit-backend-vllm hwfit-backend-sglang hwfit-backend-llamacpp hwfit-backend-ollama hwfit-backend-mlx hwfit-backend-mlx_image hwfit-backend-diffusers hwfit-backend-colibri_glm hwfit-backend-colibri_hy3">`;
      ...
      function updateCmd() {
        ...
        if (backend === 'colibri_glm' || backend === 'colibri_hy3') {
          const settings = colibriSettingsFromFields(f);
          const runtimeId = backend === 'colibri_glm'
            ? 'colibri.glm'
            : 'colibri.hy3';
          const previewSeq = (panel._colibriPreviewSeq || 0) + 1;
          panel._colibriPreviewSeq = previewSeq;
          panel._colibriPreviewError = '';
          const commandBox = panel.querySelector('.hwfit-serve-cmd');
          const preview = renderColibriCommand(runtimeId, settings)
            .then(command => {
              if (panel._colibriPreviewSeq !== previewSeq) return command;
              if (_cmdManuallyEdited) return command;
              panel._cmd = command;
              panel._host = f.host || '';
              panel._colibriSettings = settings;
              panel._colibriPreviewError = '';
              if (commandBox) {
                commandBox.value = _formatServeCmdPreview(command);
                commandBox.style.height = 'auto';
                commandBox.style.height = commandBox.scrollHeight + 'px';
              }
              return command;
            })
            .catch(error => {
              if (panel._colibriPreviewSeq !== previewSeq) return '';
              const message = error?.message || String(error);
              panel._cmd = '';
              panel._colibriPreviewError = message;
              if (commandBox) {
                commandBox.value = `Colibri preview unavailable: ${message}`;
                commandBox.style.height = 'auto';
                commandBox.style.height = commandBox.scrollHeight + 'px';
              }
              return '';
            });
          panel._colibriPreviewPromise = preview;
          return panel._cmd || '';
        }
        if (backend === 'prism') {
          const settings = prismSettingsFromFields(f);
          const previewSeq = (panel._prismPreviewSeq || 0) + 1;
          panel._prismPreviewSeq = previewSeq;
          panel._prismPreviewError = '';
          const prismCommandField = panel.querySelector('[data-field="prism_command"]');
          const commandBox = panel.querySelector('.hwfit-serve-cmd');
          const preview = renderPrismCommand(_prismModel?.id || '', settings)
            .then(command => {
              if (panel._prismPreviewSeq !== previewSeq) return command;
              if (_cmdManuallyEdited) return command;
              panel._cmd = command;
              panel._host = f.host || '';
              panel._prismSettings = settings;
              panel._prismPreviewError = '';
              if (prismCommandField) prismCommandField.value = command;
              if (commandBox) {
                commandBox.value = _formatServeCmdPreview(command);
                commandBox.style.height = 'auto';
                commandBox.style.height = commandBox.scrollHeight + 'px';
            ...
            .catch(error => {
              if (panel._prismPreviewSeq !== previewSeq) return '';
              const message = error?.message || String(error);
              panel._cmd = '';
              panel._prismPreviewError = message;
              if (prismCommandField) prismCommandField.value = '';
              if (commandBox) {
                commandBox.value = `PrismML preview unavailable: ${message}`;
                commandBox.style.height = 'auto';
                commandBox.style.height = commandBox.scrollHeight + 'px';
              }
              return '';
            });
          panel._prismPreviewPromise = preview;
          return panel._cmd || '';
        }
        if (backend === 'llamacpp') {
        ...
        if (backend !== 'prism' && f.extra && f.extra.trim()) cmd += ' ' + f.extra.trim();
      ...
      const _BACKEND_GLYPHS = {
        ...
        prism: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linejoin="round" aria-hidden="true"><path d="M12 2 3 20h18L12 2Z"/><path d="M12 2v12M3 20l9-6 9 6"/></svg>',
      ...
      function updateBackendVisibility() {
        ...
        const isPrism = b === 'prism';
        // Native engines follow vanilla Odysseus: controls regenerate the
        // textarea, and the operator can then edit/persist newer engine flags
        // before launch. Python parses and re-quotes the final argv.
        ...
        if (cmdBox) {
          cmdBox.readOnly = false;
          cmdBox.title = isPrism
            ? 'Editable final command. Changing a PrismML control regenerates this field; manual edits are parsed and shell-quoted before launch.'
            : isColibri
              ? 'Editable final command. Changing a Colibri control regenerates this field; manual edits are parsed and shell-quoted before launch.'
              : '';
        }
        const launchButton = panel.querySelector('.hwfit-serve-launch');
        if (launchButton) {
          const selectedPort = Number(panel.querySelector('[data-field="port"]')?.value || 0);
          const observedPort = Number(_prismProvider?.endpoint?.port || 0);
          const prismPortAvailable = !(
            _prismProvider?.endpoint?.port_open
            && selectedPort === observedPort
          );
          const prismSpecReady = !panel.querySelector('[data-field="prism_speculative"]')?.checked
            || !!_prismModel?.drafter?.complete;
          const prismVisionReady = !panel.querySelector('[data-field="prism_vision"]')?.checked
            || !!_prismModel?.mmproj?.complete;
          const prismReady = !!(
            _prismProvider?.source?.ready
            && _prismProvider?.build?.ready
            && _prismModel?.weights?.complete
            && prismPortAvailable
            && prismSpecReady
            && prismVisionReady
          );
          const colibriObservedPort = Number(_colibriProvider?.endpoint?.port || 0);
          const colibriPortAvailable = !(
            _colibriProvider?.endpoint?.port_open
            && selectedPort === colibriObservedPort
          );
          const colibriReady = !!(
            _colibriProvider?.source?.ready
            && _colibriProvider?.build?.ready
            && _colibriProvider?.model?.present
            && colibriPortAvailable
          );
          const ready = isColibri
            ? colibriReady
            : isPrism
            ? prismReady
            : true;
          ...
          launchButton.title = ready
            ? ''
            : `Complete the ${isPrism ? 'PrismML' : 'Colibri'} source, CUDA build, exact model, and port preflight in Dependencies before launch.`;
        }
      }
      ...
      async function updateRuntimeReadinessNote() {
        ...
        if (!['vllm', 'sglang', 'llamacpp', 'mlx', 'mlx_image', 'diffusers', 'colibri_glm', 'colibri_hy3', 'prism'].includes(backend)) {
      ...
      function _applyColibriProfile(profileId) {
        ...
        setValue('colibri_profile_env', JSON.stringify(env));
        ...
        setValue('colibri_topp', profile.topp ?? '');
        setValue('colibri_repin', profile.repin ?? '');
        setValue('colibri_cap', profile.cap ?? '');
        setValue('colibri_topk', profile.topk ?? '');
        setValue('colibri_temp', profile.temp ?? '');
        setValue('colibri_route_j', env.ROUTE_J ?? '2');
        setValue('colibri_route_m', env.ROUTE_M ?? '12');
        setValue('colibri_route_alpha', env.ROUTE_ALPHA ?? '0.5');
        ...
        setChecked('colibri_uring', env.URING === '1');
        setChecked('colibri_cache_route', env.CACHE_ROUTE === '1');
        setChecked('colibri_cuda_mtp', env.COLI_CUDA_MTP === '1' || Number(env.DRAFT || 0) > 0);
        setChecked('colibri_tensor_cores', env.COLI_CUDA_TC_W4A16 === '1');
        setChecked('colibri_cuda_dense', env.CUDA_DENSE === '1');
        setChecked('colibri_cuda_attention', env.CUDA_ATTN === '1');
        setChecked('colibri_kv_i8', env.KV_I8 === '1');
        setChecked('colibri_tool_salvage', env.COLI_TOOL_SALVAGE === '1');
      ...
      function _applyPrismProfile(profileId) {
        const profile = _prismProfiles[String(profileId || '')];
        if (!profile) return;
        _cmdManuallyEdited = false;
        const setValue = (field, value) => {
          const el = panel.querySelector(`[data-field="${field}"]`);
        ...
        setValue('prism_context', profile.context ?? 8192);
        setValue('prism_gpu_layers', profile.gpu_layers ?? 999);
        setValue('prism_parallel', profile.parallel ?? 1);
        setChecked('prism_flash_attention', profile.flash_attention !== false);
        setChecked('prism_kv4', !!profile.kv4);
        setChecked('prism_speculative', !!profile.speculative);
        updateCmd();
      }
      panel.querySelector('[data-field="prism_profile"]')?.addEventListener('change', event => {
        _applyPrismProfile(event.currentTarget.value);
      });
      ...
      function _loadSlotIntoPanel(slotIdx) {
        ...
        updateBackendVisibility();
        updateRuntimeReadinessNote();
        if (p.fields?._manual_cmd) {
          _cmdManuallyEdited = true;
          panel._cmd = String(p.cmd || p.fields._manual_cmd || '');
          const commandBox = panel.querySelector('.hwfit-serve-cmd');
          if (commandBox) {
            commandBox.value = _formatServeCmdPreview(panel._cmd);
            commandBox.style.height = 'auto';
            commandBox.style.height = commandBox.scrollHeight + 'px';
          }
        } else {
      ...
      panel.querySelectorAll('.hwfit-sf').forEach(el => {
        ...
        el.addEventListener('change', (e) => {
          ...
          if (
            e.target.dataset.field === 'port'
            || String(e.target.dataset.field || '').startsWith('prism_')
          ) {
            updateBackendVisibility();
          }
          updateCmd();
      ...
      panel.querySelector('.hwfit-serve-launch').addEventListener('click', async (ev) => {
        ...
        const _isPrismLaunch = serveState.backend === 'prism';
        if (_isColibriLaunch) {
          serveState._colibri_settings = colibriSettingsFromFields(serveState);
          if (!_cmdManuallyEdited) {
            try {
              launchCmd = _normalizeServeCmdForLaunch(
                await renderColibriCommand(
                  serveState.backend === 'colibri_glm'
                    ? 'colibri.glm'
                    : 'colibri.hy3',
              ...
              panel._colibriSettings = serveState._colibri_settings;
              panel._colibriPreviewError = '';
              if (_cmdTextarea) {
                _cmdTextarea.value = _formatServeCmdPreview(launchCmd);
                _cmdTextarea.style.height = 'auto';
                _cmdTextarea.style.height = _cmdTextarea.scrollHeight + 'px';
              }
            } catch (error) {
              _restoreLaunchBtn();
              uiModule.showToast(
                `Colibri launch settings are invalid: ${error?.message || error}`,
                9000,
              );
              return;
            }
        ...
        if (_isPrismLaunch) {
          serveState._prism_settings = prismSettingsFromFields(serveState);
          if (!_cmdManuallyEdited) {
            try {
              launchCmd = _normalizeServeCmdForLaunch(
                await renderPrismCommand(
                  serveState.prism_model_id || _prismModel?.id || '',
                  serveState._prism_settings,
                )
              );
              panel._cmd = launchCmd;
              panel._prismSettings = serveState._prism_settings;
              panel._prismPreviewError = '';
              if (_cmdTextarea) {
                _cmdTextarea.value = _formatServeCmdPreview(launchCmd);
                _cmdTextarea.style.height = 'auto';
              ...
              _restoreLaunchBtn();
              uiModule.showToast(
                `PrismML launch settings are invalid: ${error?.message || error}`,
                9000,
              );
              return;
            }
          ...
          const prismCommandField = panel.querySelector('[data-field="prism_command"]');
          if (prismCommandField) prismCommandField.value = launchCmd;
        }
            ...
            if (_clashing.length) {
              ...
              if (_choice === 'alternate') {
                ...
                if (_oldPort && _nextPort && _nextPort !== _oldPort) {
                  if (_isColibriLaunch) {
                    const _synced = synchronizeColibriLaunchPort(
                      launchCmd,
                      serveState,
                      _nextPort,
                    );
                    launchCmd = _synced.command;
                  } else if (_isPrismLaunch) {
                    const _synced = synchronizePrismLaunchPort(
                      launchCmd,
                      serveState,
                      _nextPort,
                    );
                    ...
                    serveState.port = String(_nextPort);
                  }
                  const _portField = panel.querySelector('.hwfit-sf[data-field="port"]');
                  if (_portField) _portField.value = String(_nextPort);
                  panel._cmd = launchCmd;
        ...
        const _needsGpu = ['vllm', 'sglang', 'colibri_glm', 'colibri_hy3', 'prism'].includes(serveState.backend)
        ...
        try {
          ...
          const _wantsGpu = ['llamacpp', 'vllm', 'sglang', 'diffusers', 'colibri_glm', 'colibri_hy3', 'prism'].includes(serveState.backend);
...
function _retryCachedModel(repo, m) {
  ...
  // Auto-downloads launched as part of serving favor the conservative lane;
  // users can start a high-performance Xet download explicitly from Download.
  payload.reliable_download = true;
  if (platform === 'windows') {
    if (env === 'venv' && envPath) {
