...
def test_redirect_base_override_is_forwarded_into_the_container():
    ...
    # Diogenes's GPU compose files are intentionally thin overlays and always
    # inherit the shared environment from docker-compose.yml.
    path = _repo_root() / _BASE_COMPOSE_FILE
    compose = yaml.safe_load(path.read_text(encoding="utf-8"))
    environment = set(compose["services"]["odysseus"]["environment"])
    assert "OAUTH_REDIRECT_BASE_URL=${OAUTH_REDIRECT_BASE_URL:-}" in environment
